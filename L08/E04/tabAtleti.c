...
void editPiano(tabAtleti_l ta, esercizio_l e, char *nome)
{
    ...
    case 3:
        ...
        {
            if (strcmp(getEserEs(es)->nome, nome) == 0)
            {
                printf("Inserire set e rip: ");
                scanf("%d %d", &newSet, &newRip);

                setEserSet(es, newSet);
                setEserRip(es, newRip);
...
int getNAtleti(tabAtleti_l ta)
{
    return ta->nAtleti;
}

void setNAtleti(tabAtleti_l ta, int x)
{
    ta->nAtleti = x;
}

atleta_l getHeadAtleti(tabAtleti_l ta)
{
    return ta->head;
}

void setHeadAtleti(tabAtleti_l ta, atleta_l a)
{
    ta->head = a;
}

atleta_l getTailAtleti(tabAtleti_l ta)
{
    return ta->tail;
}

void setTailAtleti(tabAtleti_l ta, atleta_l a)
{
    ta->tail = a;
}
